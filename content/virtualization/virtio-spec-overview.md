...
其中，flags取值为`VIRTQ_AVAIL_F_NO_INTERRUPT`时表示前端驱动告诉后端：“当你消耗完一个IO buffer的时候，不要立刻给我发中断”。
...
退出到KVM后利用`ioeventfd`机制通知到用户态的QEMU，QEMU主循环（main_loop poll）检测到ioeventfd事件后调用callback进行处理。
